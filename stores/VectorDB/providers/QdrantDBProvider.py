...
class QdrantDBProvider(VectorDBInterface):
    ...
    def create_collection(self,
        ...
        if not self.is_collection_existed(collection_name=collection_name):
            self.client.create_collection(collection_name=collection_name,vectors_config= models.VectorParams(
                size = embedding_size,
    ...
    def insert_one(self,collection_name:str,text:str,vector:list,
        ...
        if self.is_collection_existed(collection_name=collection_name):
            self.logger.error(f"Can't insert a new record to a non-existing collection {collection_name}")
            return False
        try:
    ...
    def insert_many(self,collection_name:str,texts:list,vectors:list,
        ...
        for i in range(0,len(texts),batch_size):
            ...
            batch_records = [
                models.Record(id =batch_record_ids[x], vector = batch_vectors[x],payload = {"text":batch_texts[x],"metadata":batch_metadata[x]})
